...
/*
  Migrates an existing governance repository to create a custom Lineage baseline.

  This app combines extract-config and import-policy to provide a complete
  migration workflow for organizations wanting to adopt Lineage.
  ...
  2. Generate appropriate .lineage.toml configuration
  3. Import supported governance files as Lineage packs
...
Migrate existing governance repositories to Lineage baseline format.
...
Options:
  ...
  --baseline-ref REF        Lineage baseline reference (default: github:Lineage-org/lineage-baseline)
...
Output:
  Creates a complete Lineage baseline in the output directory with:
  - Imported governance packs
  - Generated .lineage.toml configuration
  - Migration report
...
EOF
    ...
    for lang in "''${detected_languages[@]}"; do
      case "$lang" in
        "javascript")
          suggested_packs+=("prettier" "eslint" "javascript/eslint" "javascript/prettier" "editorconfig")
          ;;
        "python")
          suggested_packs+=("bandit" "flake8" "python/bandit" "python/flake8" "editorconfig")
          ;;
        "rust")
          suggested_packs+=("rust/clippy" "rust/rustfmt" "editorconfig")
          ;;
        "go")
          suggested_packs+=("go/gofmt" "go/golangci-lint" "editorconfig")
          ;;
      esac
    done
    ...
    log_info "Generating .lineage.toml configuration..."
    ...
    if [[ "$DRY_RUN" == false ]]; then
      ...
      cat > "$OUTPUT_DIR/.lineage.toml" << EOF
...
[languages]
# Primary language detected from project files
primary = "$primary_language"

# Additional languages detected in the project
supported = [$(printf '"%s",' "''${detected_languages[@]}" | sed 's/,$//')]
...
base = ["license", "security", "codeowners", "editorconfig"]

# Language-specific and other packs detected during migration
...
EOF
      log_success "Generated .lineage.toml"
    else
      log_info "Would generate .lineage.toml with ''${#unique_packs[@]} packs"
    fi
    ...
    if [[ "$DRY_RUN" == false ]]; then
      ...
      log_success "Copied Lineage core files"
    else
      log_info "Would copy Lineage core files from $BASELINE_REF"
    fi
...
# Generated by Lineage governance migration
#
...
**Migration Date:** $(date)
**Migration Tool:** Lineage migrate-governance
...
1. **Review Generated Configuration**
   - Examine .lineage.toml for organization settings
...
---
Generated by Lineage migrate-governance
EOF
    ...
    if [[ "$DRY_RUN" == false ]]; then
      echo "Custom Lineage baseline created at: $OUTPUT_DIR"
      echo ""
      echo "Generated baseline contains:"
      echo "  - $(find "$OUTPUT_DIR/packs" -name "*.nix" -type f 2>/dev/null | wc -l) governance packs"
      echo "  - Organization configuration (.lineage.toml)"
      echo "  - Lineage core infrastructure (flake.nix, lib/, apps/)"
